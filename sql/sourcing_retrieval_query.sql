...
-- ecomm agent runs: services/ecomm-agent/fetch_context.mjs holds the same
-- text as Q_RETRIEVAL, differing only in bind syntax (`?` positional there,
-- the documentary `:trend_id` / `:tier` here). Keep the two in sync.
-- test/sourcing_retrieval.test.sql exercises this exact shape against real
-- DIM_CATALOG_PRODUCT / FCT_TREND_ENRICHMENT_LEDGER fixture rows.
--
...
-- exists (services/lib/sourcing_run.mjs: SEMANTIC_THRESHOLD, TOP_N).
--
...
--
-- Usage (the service binds :trend_id and :tier at runtime):
--   the CTE resolves the trend's latest real vector; if it returns zero
--   rows, fetch_context.mjs treats that as "no sourceable vector yet" (see
--   services/lib/sourcing_run.mjs's design-decisions note on the resulting
--   'failed' outcome) rather than running this query at all.
