...
--
-- ALREADY APPLIED — this row is live (verified by direct query 2026-08-21).
-- This file is the registration RECORD, kept so the prompt's provenance and
-- exact text live in the repo; it is not part of any deploy or startup path
-- and does not need re-running. The WHERE NOT EXISTS guard below makes a
-- re-run a safe no-op if someone does.
--
...
-- constant in the ecomm agent (services/ecomm-agent/selector.mjs, per this
-- repo's fleet convention:
...
-- services/lib/sourcing_run.mjs) — the multi-tier top-up math that would
-- otherwise reduce it is out of scope.
--
-- The user message (trend block + numbered candidate pool) is NOT a
-- registry row — it's assembled directly in services/ecomm-agent/selector.mjs
-- by services/lib/sourcing_run.mjs's formatCandidatesForPrompt(), mirroring how
