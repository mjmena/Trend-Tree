...
def seed_lifecycle_v0(session, trend_id):
    ...
    # v2 (ADR-0005) — see lifecycle-subagent-p_gYC562o/run_subagent/entry.js.
    # Keep both in sync. At promotion time: recency_factor = 1.0 (signals just
    # linked). Breadth uses source-family count as proxy for publisher domains
    # (entropy proxy 1.0) — lifecycle agent recomputes per-publisher within ~1h.
    # Status NEW → fixed heat factor 0, so NEW_HEAT = HEAT_BASE.
    #
    # heat_base = 25*1 + 25*min(1, signal_count/3) + 40*log_score(sources)
    #           + 10*confidence
    # log_score = (log2(n) - 0.5) / (log2(6) - 0.5), clamped to [0,1].
    session.sql(f"""
        ...
        FROM (
            SELECT
                ...
                ROUND(LEAST(100, GREATEST(0,
                    25.0
                    + 25.0 * LEAST(1.0, COALESCE(ARRAY_SIZE(c.SUPPORTING_SIGNAL_IDS), 0) / 3.0)
