...
-- ════════════════════════════════════════════════════════════════════════
-- 3. audit.system v2 + audit.report_rubric v2
--    Adds {{orphan_trends_block}} + a new `data_hygiene` per-area status.
--    Tripwire after the 2026-05-26 migration-orphan purge — catches future
--    drift from the distillation agent's hallucinated/legacy SIGNAL_IDs.
-- ════════════════════════════════════════════════════════════════════════

UPDATE DIM_LLM_PROMPT SET IS_ACTIVE = FALSE
WHERE PROMPT_KEY = 'audit.system' AND VERSION = 1 AND IS_ACTIVE = TRUE;
...
lifecycle           — { status, ledger_inserts_24h, last_eval_age_minutes, gemini_cost_24h_usd }
dashboard           — { status, last_refresh_age_minutes, target_lag_minutes }
data_hygiene        — { status, active_orphan_trends }
workflow_health     — { audited_count, active_count, errored_24h: [{workflow_name, count, top_error}] }
cost_24h_usd        — total
alerts              — array of { severity: INFO|WARN|RED, area: string, summary: string, evidence: string }
slack_summary_md    — markdown body for the Slack DM (≤ 1500 chars). Lead with the headline; bullet alerts; don't paste raw JSON.
...
ORPHAN TRENDS (data-hygiene tripwire — see rubric):
{{orphan_trends_block}}
...
Do NOT propose remediation. The report is read by a human (Marty) who decides what to act on. Your job is to make the right thing obvious.$$,
    PARSE_JSON('{"max_iterations": 6, "budget_usd": 0.50, "per_call_max_tokens": 4096, "thinking_level": "medium"}'),
    TRUE,
    SHA2(CONCAT_WS(':', 'audit.system', 'v2'), 256),
    'system_seed',
    'v2 — adds {{orphan_trends_block}} placeholder + data_hygiene per-area status. Tripwire post-2026-05-26 orphan purge.'
WHERE NOT EXISTS (
    SELECT 1 FROM DIM_LLM_PROMPT WHERE PROMPT_KEY = 'audit.system' AND VERSION = 2
);

UPDATE DIM_LLM_PROMPT SET IS_ACTIVE = FALSE
WHERE PROMPT_KEY = 'audit.report_rubric' AND VERSION = 1 AND IS_ACTIVE = TRUE;
...
The `active_orphan_trends` count in the ORPHAN TRENDS block is the number of
live (non-RETIRED) trends whose FCT_TREND_SIGNALS rows all point at SIGNAL_IDs
that don't exist in FCT_SIGNALS. The migration-era backlog (110 trends) was
purged 2026-05-26 via sql/orphan_purge_2026_05_26.sql. After the purge, this
count should stay near 0; any drift indicates the distillation agent is still
emitting hallucinated/legacy SIGNAL_IDs (see
docs/handoffs/distillation-orphan-leak-2026-05-26.md).

GREEN  : active_orphan_trends == 0.
YELLOW : active_orphan_trends 1–5 (small leak from distillation; investigate but not urgent).
RED    : active_orphan_trends > 5 (the distillation root cause is producing orphans faster than expected; cleanup may be needed).
...
Do not include cost-only alerts as the basis for non-GREEN status unless RED-tier — cost is informational.$$,
    NULL,
    TRUE,
    SHA2(CONCAT_WS(':', 'audit.report_rubric', 'v2'), 256),
    'system_seed',
    'v2 — adds DATA HYGIENE area (active_orphan_trends thresholds) for the post-purge tripwire.'
WHERE NOT EXISTS (
    SELECT 1 FROM DIM_LLM_PROMPT WHERE PROMPT_KEY = 'audit.report_rubric' AND VERSION = 2
);
