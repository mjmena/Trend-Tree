...
- `DIM_LLM_PROMPT` is the other thing that deploys, live-immediate, outside git: any change to it must land as a committed `sql/update_prompts_*.sql` migration in the same session — never edit the table directly. The same commit must also bump that `PROMPT_KEY`'s entry in the manifest inside `audit-agent-p_xMC9nm3/workflow.yaml`'s `q_prompt_drift` step (both the `sql.value` and `sql.query` copies) — the daily audit compares live `DIM_LLM_PROMPT` versions against that manifest and flags anything it can't account for as a governance RED (CRMA-469).
