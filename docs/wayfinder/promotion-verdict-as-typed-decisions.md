...
- [Decide: DEFER becomes bounded and visible](https://mcclatchy.atlassian.net/browse/CRMA-1219) — **Decided:** DEFER is REMOVED entirely from promotion's verdict set — the ticket asked how to bound it, production says it should not exist. Verdict set becomes PROMOTE_NEW / MERGE_INTO_EXISTING / REJECT. The 48h hold buys NO evidence (cluster size + source count unchanged in 27/27; candidates are frozen once distillation writes them), has NEVER produced a merge (0 of 27), and the proposed cap of 3 would have destroyed a real trend (cand-6nm5r52smodzwq5t deferred 7x with 0 loop failures, then PROMOTED).
  **Binds:** OVERTURNS the standing constraint that low-confidence candidates route to DEFER. Replacements: (1) a machine no-decision is an ERROR not a verdict — the three fallbacks at run_subagent/entry.js:642/:699/:717 write a distinct DECISION value plus a ledger row, because without the row the retry count is underivable and MODEL_USED cannot separate provenance (CRMA-1216); (2) machine failures retry with NO hold (promotion runs ~6h, median gap 360min), bounded at 3 attempts, then PARK — do NOT force REJECT on exhaustion even though 3/3 measured failures ended REJECT, because an outage would mass-reject every candidate in flight; (3) new REJECT category INSUFFICIENT_EVIDENCE absorbs NEEDS_MORE_SIGNAL (33 rows/24 candidates, deliberately chosen) and carries the REVISIT DISPOSITION — it is the only rejection meaning 'plausible, we let it go'; add one reason, not a family; derive the disposition in code; (4) counters DERIVE from the append-only ledger, nothing gains a column; (5) audit agent gains a parked-candidate check — its distillation_pending check at audit-agent workflow.yaml:86-95 EXCLUDES held candidates by construction, so no alarm exists today; (6) DEFERRED_UNTIL/DEFER_REASON keep their columns, stop being written, and the claim filter at promotion workflow.yaml:41/:69 goes — safe ONLY because the held population is currently zero, re-check before shipping; (7) DELETE prompt principle 6 entirely (seed_prompts_promotion.sql:83), 'when in doubt DEFER' included, and NEVER tell the model a defer or retry count (CRMA-1217 measured outcome-naming bias 0.61 to 0.74); (8) ships BEFORE the Cloud Run move as its own Pipedream change, so the lift-and-shift stays a pure runtime move. VERIFICATION STATUS: decided, NOT proven — no replay has shown what the agent decides on first look without DEFER, so CRMA-1229/CRMA-1222 become a REGRESSION CHECK; if the 12 post-defer promotions collapse into rejections, revisit. decision_category space moves 10 to 9 values. Rejected topics already resurface WITHOUT a hold: 8 of 15 deferred-then-rejected topics match a later-promoted trend at >=0.80 cosine (arctic-embed-m on topic strings, NOT the pipeline's embedding space — directional only), via re-clustering in distillation-revisit which yields a NEW fatter candidate with real new evidence.
