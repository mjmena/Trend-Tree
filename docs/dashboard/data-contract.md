...
**Source of truth:** the table DDL in the Trend-Tree repo — `sql/dt_trend_dashboard.sql`, `sql/dt_trend_daily.sql`, `sql/dt_trend_connections.sql` (+ `sql/fct_trend_connections_ledger.sql`), `sql/task_recompute_content_matches.sql` (+ `sql/fct_trend_content_matches_ledger.sql`), `sql/fct_trend_sourcing_ledger.sql` (+ `sql/fct_trend_sourcing_candidates.sql`), `sql/fct_prediction_verdict_ledger.sql`. This page is the canonical engineer-facing schema reference. **Database:** `MCC_PRESENTATION.TREND_AGENT` · **Account:** `WVB49304-MCCLATCHY_EVAL`. **Last updated:** 2026-08-24.
...
| Column | Type | What it is | Example value |
| --- | --- | --- | --- |
| `SOURCING_STATUS` | VARCHAR | State of the **latest** sourcing run for this trend. One of `not_sourced` (no run has ever been recorded), `running` (a run is in flight — or crashed mid-run: a `running` header is only superseded when the next poll tick opens a new run, so don't treat it as a state that always resolves on its own), `matched` (the selector picked at least one product), `no_match` (the run completed and nothing cleared the bar), `failed` (the run errored; the poller retries it on the next tick). **Never `NULL`** — a trend with no header reads `not_sourced`, so "never sourced" and "sourced, found nothing" are always distinguishable in one `SELECT`. | `matched` |
| `SOURCED_PRODUCTS` | ARRAY | The selector's **picks** from that latest run, `SEMANTIC_SCORE` desc. Shape: `{ tier, catalog_product_id, product_handle, product_title, product_type, vendor, product_url, price_at_match, image_url_at_match, available_at_match, semantic_score, reasoned_fit, reasoned_fit_rationale }`. `NULL` (not `[]`) when the latest run picked nothing — and also while a re-source is in flight, because a new header opens as `running` with no candidate rows yet, so a previously matched trend's products blank out until that run completes. Matches `NEAREST_CONTENT` / `RELATED_TRENDS`' null-when-absent convention. Every key is always present, explicitly `null` when unset — the entry shape does not vary row to row. | `[{"tier":"shopify","product_handle":"lip-jelly-hydrating-peptide-lip-treatment-…","product_title":"Lip Jelly: Hydrating Peptide Lip Treatment…","vendor":"Beauty Care Bag","semantic_score":0.4989,"reasoned_fit":"strong","reasoned_fit_rationale":"A peptide-infused lip treatment formulated with a bouncy, hydrating jelly texture…"}]` |
| `SOURCED_AT` | TIMESTAMP | When that latest sourcing run completed, so staleness is visible. `NULL` while a run is still `running`, and `NULL` for a trend that has never been sourced. | `2026-08-21T13:20:54Z` |

**Picks only — rejects are not here.** A run shows the selector a pool of retrieval candidates and it picks a subset; `SOURCED_PRODUCTS` carries only `SELECTED = TRUE`. The shown-but-rejected candidates stay in `FCT_TREND_SOURCING_CANDIDATES`, where they are retrieval-calibration evidence (replaying `SEMANTIC_SCORE` against actual selector outcomes is how the threshold gets re-tuned) — read them from the ledger, not from the dashboard.

**Two independent judgements, never blended.** `semantic_score` is retrieval geometry — `VECTOR_COSINE_SIMILARITY` between the trend's `TREND_VECTOR` and the product's embed vector, reproducible from stored data. `reasoned_fit` is the selector model's verdict, an enum (`strong` / `partial` / `weak`), never a number. They are separate fields by design; do not combine them into a single relevance score. Ordering is on `semantic_score` alone.
