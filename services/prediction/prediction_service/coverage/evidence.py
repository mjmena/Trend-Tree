...
from .posture import ExternalDemand, coverage_demotes, posture_for
...
def build_coverage_evidence(
    ...
    return {
        ...
        "posture": posture_for(reading, demand=external),
        "demoted": demoted,
...
def record_coverage(
    ...
    *,
    ...
    A ``None`` reading leaves the evidence exactly as it was, which for a
    re-evaluation means the prior row's ``coverage`` is carried forward
    (matching/run.py already copies it). An unwired service must not
    overwrite a real detection with a "we did not look" miss, for the same
    reason the sweep refuses to downgrade a real saturation reading.
    """
    if reading is None:
