...
// the retrieval pool the service already fetched and the selector's raw
// emit tool payload and derives the exact ledger write plan the service
...
//
// This module is imported directly by services/ecomm-agent (Cloud Run
// bundles the whole services/ tree into the image — see
// services/ecomm-agent/Dockerfile, whose build context is the repo root).
// There are no inlined copies: the Pipedream-era constraint that forced
// this logic to be duplicated into every step file is gone with the
// platform move (CRMA-776's Cloud Run respec). It stays a separate pure
// module so the core logic is unit-testable without the HTTP shell,
// Snowflake, or Gemini — mirroring agents/lib/promotion_gate.mjs's role
// for the promotion lead, and living under services/lib/ per CRMA-439's
// shared-module home for the GCP fleet.
//
